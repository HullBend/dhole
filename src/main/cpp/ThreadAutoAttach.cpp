 ...
 : m_vm(NULL), m_env(NULL)
{
    ...
    if (m_vm) {
        JavaVMAttachArgs attachArgs = {JNI_VERSION_1_4, "dhole native thread", NULL};
        jint rc = 0;
        try {
            ...
            m_vm = NULL;
            throw JException("ThreadAutoAttach::ThreadAutoAttach() - JNI AttachCurrentThreadAsDaemon failed at pos 1");
        }
        if (rc != 0) {
            m_vm = NULL;
            throw JException("ThreadAutoAttach::ThreadAutoAttach() - JNI AttachCurrentThreadAsDaemon failed at pos 2");
        }
        if (!m_env) {
            detach();
